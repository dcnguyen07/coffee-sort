...
public class BoardManager : MonoBehaviour
{
    ...
    private List<PlaceModel> activeTrays = new List<PlaceModel>();
    ...
    private void HandleTrayDrag()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hits.Length > 0)
            {
                ...
                if (topTray != null)
                {
                    trayStartPosition = topTray.transform.position;
    ...
    {
        currentBoardState = BOARD_STATE.IDLE;
        maxSlots = 7;
        availableSlots = 4;
        totalSlots = 7;
        unlockedSlots = 4;
    ...
    public void SetLevelData(LevelDataSO levelData)
    {
        if (levelData == null)
        {
            Debug.LogError("❌ LevelDataSO không hợp lệ!");
            return;
        }

        Debug.Log($"📌 Đang thiết lập dữ liệu cho Level {levelData.level}");

        // 🔥 1. Cập nhật level hiện tại
        currentLevel = levelData;
        Debug.Log($"✅ Thiết lập level {levelData.level} thành công!");
    }
    ...
    private void TryPlaceTrayInSlot(PlaceModel tray)
    {
        foreach (BoardSlot slot in boardSlots)
        {
            if (slot.IsAvailable())
            {
                ...
                currentBoardState = BOARD_STATE.PLAYING;
                UpdateBoosterButtonStates();
    ...
    private void MoveQueue()
    {
        ...
        currentBoardState = BOARD_STATE.PLAYING;  // Set state to PLAYING when queue starts moving
        UpdateBoosterButtonStates();
    ...
    public void DoMoveCup(CupModel cup, int index, bool lastCup)
    {
        ...
        cup.transform.DOMove(cupSpawnPoints[index].position, 0.1f).SetEase(Ease.OutQuad).OnComplete(() =>
        {
            if (lastCup)
            {
                currentBoardState = BOARD_STATE.IDLE;  // Set state to IDLE when queue movement completes
                UpdateBoosterButtonStates();
                MoreCupInQueue();
            }
        });
    ...
    {
        return !DOTween.IsTweening(this) && moveQueueCompleted && currentBoardState == BOARD_STATE.IDLE;
    }
